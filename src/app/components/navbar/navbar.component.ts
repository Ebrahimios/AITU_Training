...
import { AuthService, User, FirebaseFactory } from '../../services/firebase.service';
...
@Component({
    ...
    imports: [CommonModule, RouterModule, MatDialogModule, LanguageSwitcherComponent]
})
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.factoryRequestsSubscription = this.authService.factoryRequests$.subscribe(factories => {
      // Filter only non-approved factory requests
      const pendingFactories = factories.filter(factory => !factory.isApproved);
      
      // Convert factory requests to notifications
      const factoryNotifications: Notification[] = pendingFactories.map(factory => ({
        id: factory.id,
    ...
    if (this.currentUser) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result && result.imageUrl) {
          ...
          console.log('Profile updated at:', new Date(result.timestamp).toLocaleString());
        }
      });
    }
  }

  handleFactoryRequest(notificationId: string | number, action: 'accept' | 'deny') {
    if (typeof notificationId === 'string') {
      // Call Firebase service to handle the factory request
      this.authService.handleFactoryRequest(notificationId, action).then(success => {
        if (success) {
  ...
  showFactoryDetails(factoryName: string | undefined) {
    ...
    const pendingFactories = this.authService.getFactoryRequests().filter(f => !f.isApproved);
